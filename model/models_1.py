...
class KD_1_only(nn.Module):
    def __init__(self, vision_dim=2048, module_dim=768, word_dim=300, vocab=None, num_of_nodes=8, graph_module='GAT',
                 graph_layers=4, unit_layers=1):
        super(KD_1_only, self).__init__()
        # kd output_unit
        self.num_classes = len(vocab['answer_token_to_idx'])  # ans_vocabsize
        self.output_unit = SimpleOutputUnitOpenEnded_KD(module_dim=module_dim, num_answers=self.num_classes)
...
class KD_1(nn.Module):
    def __init__(self, vision_dim=2048, module_dim=768, word_dim=300, vocab=None, num_of_nodes=8, graph_module='GAT',
                 graph_layers=4, unit_layers=1):
        super(KD_1, self).__init__()
    ...
    def forward(self, video_appearance_feat, video_motion_feat, question,
        ...
        video_appearance_feat = self.visual_appearance_input_unit(
            video_appearance_feat)  # appearance feature-LSTM: [Tensor] batch_size * num_of_clips * module_dim
        video_motion_feat = self.visual_motion_input_unit(
            video_motion_feat)  # motion feature-Linear: [Tensor] batch_size * num_of_clips * module_dim
        # Reasoning module
        ## 1. layer-1
...
class DualVGRUnit_multiple(nn.Module):  # 16, 8, 1, 2048, 512
    def __init__(self, word_dim=300, module_dim=512, num_of_nodes=8, appearance_graph_layers=1, motion_graph_layers=1,
                 graph_module='GAT', unit_layers=1):
        ...
        self.queryPunish_appear = nn.ModuleList(
            [QueryPunish(word_dim=word_dim, module_dim=module_dim) for _ in range(unit_layers)])
        self.queryPunish_motion = nn.ModuleList(
            [QueryPunish(word_dim=word_dim, module_dim=module_dim) for _ in range(unit_layers)])
        if graph_module == 'GAT':
            self.appearance_GCN = nn.ModuleList([punishGAT(module_dim, module_dim // 4, dropout=0.15, alpha=0.01,
                                                           n_heads=4) for _ in range(
                unit_layers * appearance_graph_layers)])  # n_feat, n_hid, dropout, alpha, n_heads
            self.motion_GCN = nn.ModuleList(
                [punishGAT(module_dim, module_dim // 4, dropout=0.15, alpha=0.01, n_heads=4) for _ in
                 range(unit_layers * motion_graph_layers)])
            self.acGCN = nn.ModuleList([punishGAT(module_dim, module_dim // 4, dropout=0.15, alpha=0.01,
                                                  n_heads=4) for _ in range(
                unit_layers * appearance_graph_layers)])  # n_feat, n_hid, dropout, alpha, n_heads
            self.mcGCN = nn.ModuleList(
                [punishGAT(module_dim, module_dim // 4, dropout=0.15, alpha=0.01, n_heads=4) for _ in
                 range(unit_layers * motion_graph_layers)])
    ...
    def forward(self, appearance_video_feat, motion_video_feat, dynamic_question_embedding, word_embedding,
                question_len):
        """
        Args:
        ...
        for i in range(self.layers):
            ...
            cycle_question_embedding, attn_score = self.queryAttn[i](word_embedding, dynamic_question_embedding,
                                                                     question_len)  # (batch_size, word_dim)
            cycle_appearance_scores = self.queryPunish_appear[i](cycle_question_embedding, aq_fusion)
            cycle_motion_scores = self.queryPunish_motion[i](cycle_question_embedding,
                                                             mq_fusion)  # (batch_size, num_of_clips, module_dim)
