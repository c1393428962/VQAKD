...
def train(cfg):
    ...
    for epoch in range(start_epoch, cfg.train.max_epochs):
        ...
        for i, batch in enumerate(iter(train_loader)):
            ...
            with torch.no_grad():
                if cfg.model_type == 'DualVGR':
                    teacher_logits, _, _, _, _, _, _ = teacher_model(*batch_input) #attn,appear_scores,mot_scores,
                else:
            ...
            if cfg.model_type == 'DualVGR':
                ...
                distillation_loss = soft_criterion(
                    F.softmax(logits / cfg.temperature, dim=1),
                    F.softmax(teacher_logits / cfg.temperature, dim=1)
                )
                loss = cfg.kd_alpha*loss+distillation_loss # TODO
    ...
    if cfg.dataset.name == 'msvd-qa' or cfg.dataset.name == 'msrvtt-qa':
        test_acc, *test_output = validate(cfg, model, test_loader, device, write_preds=False)
        ...
        resp = requests.post("https://www.autodl.com/api/v1/wechat/message/push",
                             json={
                                 "token": "69fcd3bf894d",
                                 "title": "kd_{}_{}".format(cfg.temperature, cfg.kd_alpha),
                                 "name": "A40",
                                 "content": "test Accurancy: {}!!!".format(test_acc)
                             })
...
def main():
    ...
    parser.add_argument('--temperature', dest='temperature', help='kd temperature', default=1, type=int)
    parser.add_argument('--kd_alpha', dest='kd_alpha', help='kd alpha', default=3, type=int)
